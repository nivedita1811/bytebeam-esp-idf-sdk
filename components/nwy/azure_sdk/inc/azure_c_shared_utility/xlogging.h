...
#elif (defined MINIMAL_LOGERROR1)
#define LOG(...)
#define LogInfo(...)
#define LogBinary(...)
...
#else
#define LOG(log_category, log_options, format, ...) nwy_open_log(OSI_LOG_LEVEL_DEBUG, format, ## __VA_ARGS__)
#endif

#if defined _MSC_VER
#define LogInfo(FORMAT, ...) do{LOG(AZ_LOG_INFO, LOG_LINE, FORMAT, __VA_ARGS__); }while((void)0,0)
#else
#define LogInfo(FORMAT, ...) nwy_open_log(OSI_LOG_LEVEL_INFO, FORMAT, ## __VA_ARGS__)
#endif
...
#else // _MSC_VER
#define LogError(FORMAT, ...) nwy_open_log(OSI_LOG_LEVEL_ERROR, FORMAT, ## __VA_ARGS__)
#ifdef WIN32
